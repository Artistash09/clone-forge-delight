import { Link } from "react-router-dom";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";

const comparisonRows = [
  { feature: "Email verification method", them: "Pattern matching (unverified)", us: "6-step SMTP verification", usWin: true, themLose: true },
  { feature: "Email accuracy", them: "~70% at scale", us: "98% verified", usWin: true, themLose: true },
  { feature: "Mobile coverage", them: "~20M", us: "60M direct dials", usWin: true, themLose: true },
  { feature: "Data freshness", them: "Monthly", us: "Weekly", usWin: true, themLose: true },
  { feature: "Pricing at scale", them: "Free tier + $49–$99/mo (limits drop quality)", us: "From $49/mo, consistent quality", usWin: true, themLose: true },
  { feature: "API rate limits", them: "Strict on all plans", us: "10k calls/mo (Growth), Unlimited (Enterprise)", usWin: true, themLose: true },
  { feature: "Free trial", them: "✓ (limited)", us: "✓ 50 free credits", usWin: false, themLose: false },
  { feature: "Chrome extension", them: "✓", us: "✓", usWin: false, themLose: false },
  { feature: "GDPR compliant", them: "✓", us: "✓", usWin: false, themLose: false },
];

export default function VsApollo() {
  return (
    <div className="min-h-screen bg-background">
      <Navbar />
      <div className="pt-20 pb-10 px-6 text-center max-w-[900px] mx-auto">
        <div className="inline-block bg-il-green/10 border border-il-green/30 rounded-full px-5 py-2 text-green-400 text-sm font-semibold mb-6">
          ✓ Higher accuracy. Better mobiles. Same price.
        </div>
        <h1 className="section-title">Inbound Labs vs Apollo.io</h1>
        <p className="text-il-gray-light text-[16px] leading-relaxed mt-4">
          Apollo's free tier is popular but data quality drops sharply at scale. Apollo has 220M contacts but 60%+ are unverified emails. Inbound Labs mobile number coverage is 3× Apollo's.
        </p>
      </div>

      <div className="max-w-[900px] mx-auto px-6 py-12">
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-il-border">
                <th className="text-left text-sm text-il-gray-light font-semibold py-3 px-4 w-[35%]">Feature</th>
                <th className="text-center text-sm text-il-gray-light font-semibold py-3 px-4">Apollo.io</th>
                <th className="text-center text-sm font-semibold py-3 px-4 bg-il-violet/10 text-il-violet-light">Inbound Labs</th>
              </tr>
            </thead>
            <tbody>
              {comparisonRows.map((row, i) => (
                <tr key={i} className={`border-b border-il-border ${i % 2 === 1 ? "bg-il-violet/[0.02]" : ""}`}>
                  <td className="text-sm text-il-gray-light py-3 px-4">{row.feature}</td>
                  <td className={`text-sm text-center py-3 px-4 ${row.themLose ? "text-red-400" : "text-il-gray-light"}`}>{row.them}</td>
                  <td className={`text-sm text-center py-3 px-4 bg-il-violet/[0.03] ${row.usWin ? "text-green-400 font-semibold" : "text-il-gray-light"}`}>
                    {row.us} {row.usWin && "✓"}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </div>

      <div className="max-w-[900px] mx-auto px-6 py-12">
        <h2 className="section-title" style={{ fontSize: "28px" }}>Where Apollo still wins</h2>
        <p className="text-il-gray-light text-sm mb-6">We believe in honest comparisons:</p>
        <div className="space-y-4">
          {[
            "Apollo has a generous free tier that's great for individual prospectors just getting started",
            "Apollo's built-in email sequencing tool means one less subscription for small teams",
            "Apollo's community and content ecosystem is larger",
          ].map((point) => (
            <div key={point} className="flex gap-3 items-start">
              <span className="text-il-gray-light mt-0.5">•</span>
              <p className="text-il-gray-light text-sm leading-relaxed">{point}</p>
            </div>
          ))}
        </div>
      </div>

      <div className="max-w-[900px] mx-auto px-6 py-12">
        <h2 className="section-title" style={{ fontSize: "28px" }}>Who should switch</h2>
        <div className="grid md:grid-cols-3 gap-5 mt-8">
          {[
            { emoji: "🎯", title: "Teams scaling past free tier", desc: "Apollo's data quality drops at scale. If your bounce rate is climbing, it's time to switch." },
            { emoji: "📱", title: "Anyone who needs mobile numbers", desc: "Apollo's mobile coverage is ~20M. Ours is 60M with a 30% pick-up rate." },
            { emoji: "🔬", title: "Data-driven sales teams", desc: "If you care about verified data over volume, Inbound Labs is built for you." },
          ].map((card) => (
            <div key={card.title} className="bg-il-dark2 border border-il-border rounded-xl p-6">
              <div className="text-3xl mb-3">{card.emoji}</div>
              <h3 className="text-il-white font-semibold text-[15px] mb-2">{card.title}</h3>
              <p className="text-il-gray-light text-sm leading-relaxed">{card.desc}</p>
            </div>
          ))}
        </div>
      </div>

      <div className="bg-il-dark border-t border-il-border py-16 px-6 text-center mt-12">
        <h2 className="section-title" style={{ fontSize: "28px" }}>Try Inbound Labs free — no credit card, no sales call</h2>
        <Link to="/pricing" className="inline-block mt-6 bg-il-violet hover:bg-il-violet-light text-white font-semibold px-8 py-3.5 rounded-lg transition-colors">
          Start Free
        </Link>
      </div>
      <Footer />
    </div>
  );
}
